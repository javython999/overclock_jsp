...
<head>
<meta charset="UTF-8">
<link href="${pageContext.request.contextPath}/style/noticeDetail.css" rel="stylesheet" />
...
<title>공지사항</title>
 ...
 <body>
        ...
        <section id="content">
        	<div class="contentWrap">
        		...
        		<div class="btnBox">
        			...
        			<form class="prevBtn" action="/notice/pagemove" method="get">
        			...
        			<!-- 글 목록 버튼 -->
        			<a href="/notice/list"><i class="far fa-list-alt"></i></a>
        			<!-- 다음 글 버튼 -->
        			<c:if test="${lastId > currentId}">
        			<form class="nextBtn" action="/notice/pagemove" method="get">
        		...
        		<div class="commentBox">
        			...
        			<c:if test="${!empty sessionScope.sessionID}">
        			<div>
        				<form method="post" action="/notice/comment">
        			...
        			<c:set var="userCommentTest" value="${sessionScope.sessionID}"/>
        			<div class="commentList clear">
        				<c:forEach var="c" items="${c}">
        				<div class="comment">
        				<form action="/notice/comment/delete" method="get" class="Target">
        				<input type="hidden" value="${c.writerId}" />
						<input type="hidden" name="cmtId" value="${c.id}" class="cmtId"/>
						<input type="hidden" value="${c.content}" class="originContent"/>
						<input type="hidden" name="pageId" value="${n.id}" class="pageId"/>
						</form>
						<div class="commentNick">
							${c.nickname}
							<c:if test="${userCommentTest == c.writerId}">
							<input type="button" value="수정" name="a" onclick="cmtModify(event);"/>
							<input type="button" value="삭제" onclick="askDelete();"/>							
							</c:if>
						</div>
    ...
    <script type="text/javascript" src="${pageContext.request.contextPath}/js/noticeDetail.js"></script>
  </body>
</html>
